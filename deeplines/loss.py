...
class DeepLineLoss(nn.Module):
    ...
    def forward(self, pred, gt):
        ...
        loss_center = 0.5 * self.mse(objectness * pred[:, :, 1:3], objectness * regression[:, :, :2])
        loss_angle = 0.5 * self.mse(objectness * pred[:, :, 3:4], objectness * regression[:, :, 2:3])
        loss_length = 1 * self.mse(objectness * pred[:, :, 4:], objectness * regression[:, :, 3:])
